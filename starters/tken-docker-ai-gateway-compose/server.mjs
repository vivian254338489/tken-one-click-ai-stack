...
app.get("/health", (_req, res) => res.json({ ok: true, service: "tken-docker-ai-gateway-compose", upstreamBaseUrl, routes: Object.keys(routes) }));
app.get("/v1/models", (_req, res) => res.json({ object: "list", data: Object.keys(routes).map((id) => ({ id, object: "model", owned_by: "tken-docker-gateway" })) }));
...
app.post("/v1/chat/completions", async (req, res) => {
  ...
  if (!upstreamApiKey || upstreamApiKey === "your_tken_api_key") {
    return res.json({ id: `chatcmpl-demo-${Date.now()}`, object: "chat.completion", created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, message: { role: "assistant", content: `Docker gateway demo is working. Route: ${requestedModel}.` }, finish_reason: "stop" }] });
  }
...
app.listen(port, () => console.log(`Docker AI Gateway listening on http://127.0.0.1:${port}`));
